...
void ParallelCoord::set_bounds(int x, int y, int width, int height)
{
	...
	axis_width = 10;
	axis_height = 0.8f * plot_height; // 70% plot height
	axis_y = 0.13f * plot_height;
	meta_data_height = 0.1f * plot_height;
...
void ParallelCoord::set_column_values(vector<vector<ColumnValue*>> c_values)
{
	...
	for (int i = 0; i < column_values[0].size(); i++) {
		unsigned *brushed_mask_clone = new unsigned();
		*brushed_mask_clone = brushed_mask;
		data_set_mask.push_back(brushed_mask_clone);
	}
}
...
void ParallelCoord::draw()
{
	...
	if (mouse_mode == ParallelCoord::MOUSE_MODE_LABEL_DRAG) {
		...
		ofSetColor(zoom_outside_bg_color);
		ofDrawRectangle(15, -(zoom_height+15), 0.42f*plot_width, zoom_height);
		...
		for (int k = zoom->min_index; k <= zoom->max_index; k++) {
			...
			verdana_zoom_label.drawString(str,
				15 + 6,
				-(zoom_height + 15) + (zoom_rect_offset_y * (k - zoom->min_index + 1)) + 6);
		}
	}
...
vector<unsigned*> ParallelCoord::get_bursh_mask()
{
	return data_set_mask;
}
